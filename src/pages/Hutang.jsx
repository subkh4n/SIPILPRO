...
export default function Hutang() {
  const { purchases, vendors, cashBalance, getTotalDebt, getVendor, payDebt } = useData();
  ...
  const handlePay = (purchase) => {
    if (window.confirm(`Bayar hutang ${purchase.invoiceNo} sebesar ${formatCurrency(purchase.total)}?`)) {
  ...
  return (
    <div className="animate-in">
      <div className="page-header">
        ...
        <p className="page-subtitle">Accounts Payable - Monitoring hutang vendor</p>
      </div>
      ...
      {/* Table */}
      <div className="table-container mb-6" style={{ marginBottom: 'var(--space-6)' }}>
          ...
          <tbody>
            {filteredPurchases.length === 0 ? (
              ...
              filteredPurchases.map(purchase => {
                ...
                const dueStatus = purchase.status === 'unpaid' ? getDueDateStatus(purchase.dueDate) : null;
                    ...
                    <td>
                      {purchase.status === 'unpaid' ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                          <span>{format(new Date(purchase.dueDate), 'dd/MM')}</span>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
        <div className="card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
            ...
            <div>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Total Hutang Berjalan</p>
              <p style={{ fontSize: '1.5rem', fontWeight: '700', color: 'var(--warning-500)' }}>
        ...
        <div className="card" style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
            ...
            <div>
              <p style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Saldo Kas Saat Ini</p>
        ...
        <div className="alert alert-danger" style={{ marginTop: 'var(--space-6)' }}>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.875rem', marginTop: '4px' }}>
              Total hutang melebihi saldo kas. Selisih: {formatCurrency(totalDebt - cashBalance)}
            </p>
          </div>
        </div>
      )}
